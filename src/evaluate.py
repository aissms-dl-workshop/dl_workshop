...
def evaluate(config_file):
    ...
    sns.heatmap(confusion_matrix(test_set.classes, y_pred), annot=True, cmap='Blues')
    plt.xlabel("Actual")
    ...
    df = pd.DataFrame(classification_report(test_set.classes, y_pred, target_names=target_names, output_dict=True)).transpose()
...
if __name__ == '__main__':
    ...
    a = evaluate(config=parsed_args.config)
